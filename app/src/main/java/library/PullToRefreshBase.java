...
import library.internal.Utils;
import library.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    T mRefreshableView;
    private FrameLayout mRefreshableViewWrapper;//正在刷新视图
    ...
    public final boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                if (mIsBeingDragged) {
                    mIsBeingDragged = false;
                    if (mState == State.RELEASE_TO_REFRESH) {//释放进行刷新操作
                        if (null != mOnRefreshListener) {
                            ...
                            setState(State.REFRESHING, true);
                            if (mCurrentMode == Mode.PULL_FROM_START) {//下拉刷新开始
                                mOnRefreshListener2.onPullDownToRefresh(this);
                            } else if (mCurrentMode == Mode.PULL_FROM_END) {//上拉刷新开始
    ...
    final void setState(State state, final boolean... params) {
        ...
        switch (mState) {
            case RESET:
                onReset();
                break;
            case PULL_TO_REFRESH://下拉进行刷新
                onPullToRefresh();
                break;
            case RELEASE_TO_REFRESH://释放去刷新
                onReleaseToRefresh();
                break;
            case REFRESHING://进入刷新
            case MANUAL_REFRESHING://手动刷新
                onRefreshing(params[0]);//调用父类进行刷新操作
                break;
            case OVERSCROLLING:
                // NO-OP
                break;
        ...
        if (doScroll) {
            if (mShowViewWhileRefreshing) {
                switch (mCurrentMode) {
                    ...
                    case PULL_FROM_END: //下拉刷新
                        smoothScrollTo(getFooterSize());
                        break;
                    default:
                    case PULL_FROM_START://上拉刷新
                        int size = getHeaderSize();
                        if (size != 0) {
                            size = -size;
                        } else {//还未准备好之前计算view的高度
                            int widthMeasureSpec = MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED);
                            int heightMeasureSpec = MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED);
                            mHeaderLayout.measure(widthMeasureSpec, heightMeasureSpec);
                            size = -mHeaderLayout.getMeasuredHeight();
                        }
                        smoothScrollTo(size);
                        break;
                }
     ...
     * 设置刷新显示不可见
     */
    public final void setLayoutInvisible() {
...
/*
    ...
    protected final void refreshLoadingViewsSize() {
        final int maximumPullScroll = getMaximumPullScroll();//(int) (getMaximumPullScroll() * 1.2f);
 ...
 */
        // Styleables from XML
//	  int[] aa =  new int[]{
//                getContext().getResources().getIdentifier("disabled", "styleable", getContext().getPackageName()),
//                getContext().getResources().getIdentifier("pullFromStart", "styleable", getContext().getPackageName()),
//                getContext().getResources().getIdentifier("pullFromEnd", "styleable", getContext().getPackageName()),
//                getContext().getResources().getIdentifier("both", "styleable", getContext().getPackageName()),
//                getContext().getResources().getIdentifier("manualOnly", "styleable", getContext().getPackageName()),
//                getContext().getResources().getIdentifier("pullDownFromTop", "styleable", getContext().getPackageName()),
//                getContext().getResources().getIdentifier("pullUpFromBottom", "styleable", getContext().getPackageName())
//	    };
//		TypedArray a = context.obtainStyledAttributes(attrs,aa);

//		int[] aa = new int[]{
//              getContext().getResources().getIdentifier("adapterViewBackground", "styleable", getContext().getPackageName()),
//              getContext().getResources().getIdentifier("headerBackground", "styleable", getContext().getPackageName()),
//              getContext().getResources().getIdentifier("headerTextColor", "styleable", getContext().getPackageName()),
//              getContext().getResources().getIdentifier("mode", "styleable", getContext().getPackageName()),
//              getContext().getResources().getIdentifier("headerType", "styleable", getContext().getPackageName()),
//              getContext().getResources().getIdentifier("ptrRefreshableViewBackground", "styleable", getContext().getPackageName()),
//              getContext().getResources().getIdentifier("ptrHeaderBackground", "styleable", getContext().getPackageName())
//		};
//		TypedArray a = context.obtainStyledAttributes(attrs,aa);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PullToRefresh);
//		if (a.hasValue(MResource.getIdByName(getContext().getApplicationContext(), "styleable", "PullToRefresh_ptrMode"))) {
//			mMode = Mode.mapIntToValue(a.getInteger(MResource.getIdByName(getContext().getApplicationContext(), "styleable", "PullToRefresh_ptrMode"), 0));
//		}
		...
		/*refreshLoadingViewsSize();
		refreshRefreshableViewSize(getMeasuredWidth(),getMeasuredHeight());*/

        /**
         * Styleables from XML
         */
//		if (a.hasValue(MResource.getIdByName(getContext().getApplicationContext(), "styleable", "PullToRefresh_ptrRefreshableViewBackground"))) {
        if (a.hasValue(R.styleable.PullToRefresh_ptrRefreshableViewBackground)) {
//			Drawable background = a.getDrawable(MResource.getIdByName(getContext().getApplicationContext(), "styleable", "PullToRefresh_ptrRefreshableViewBackground"));
            Drawable background = a.getDrawable(R.styleable.PullToRefresh_ptrRefreshableViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
//		} else if (a.hasValue(MResource.getIdByName(getContext().getApplicationContext(), "styleable", "PullToRefresh_ptrAdapterViewBackground"))) {
        } else if (a.hasValue(R.styleable.PullToRefresh_ptrAdapterViewBackground)) {
            Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
//			Drawable background = a.getDrawable(MResource.getIdByName(getContext().getApplicationContext(), "styleable", "PullToRefresh_ptrAdapterViewBackground"));
...
//		if (a.hasValue(MResource.getIdByName(getContext().getApplicationContext(), "styleable", "PullToRefresh_ptrScrollingWhileRefreshingEnabled"))) {
//			mScrollingWhileRefreshingEnabled = a.getBoolean(
//					MResource.getIdByName(getContext().getApplicationContext(), "styleable", "PullToRefresh_ptrScrollingWhileRefreshingEnabled"), false);
...
//		}
    ...
    private int getMaximumPullScroll() {
        switch (getPullToRefreshScrollDirection()) {
            ...
            case VERTICAL:
            default:
                return Math.round(getMeasuredHeight() / FRICTION);
        }
    }
    ...
    public static enum Mode {
        ...
        // The modeInt values need to match those from dmsj_attrs.xml
        Mode(int modeInt) {
            mIntValue = modeInt;
        }
